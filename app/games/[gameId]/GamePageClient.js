...
import { iconMapList } from "@components/screens/quests/QuestFeed";
            ...
            value={
              <div className="flex flex-wrap  gap-3 mt-1">
                {gameInfo?.socials?.map((socialObj, index) => {
                  const Icon = iconMapList[socialObj?.name];
                  return (
                    <a
                      key={socialObj?.id}
                      href={socialObj?.url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="rounded-full text-[var(--textTwo)]  text-base hover:text-[var(--textOne)]  transition-all duration-500 ease-in-out"
                    >
                      {Icon ? (
                        <Icon size={30} className="text-sm  w-6 h-6" />
                      ) : (
                        <CardChip
                          key={socialObj?.id}
                          label={socialObj?.name}
                          style={{
                            width: "fit-content",
                            minWidth: 90,
                            width: "auto",
